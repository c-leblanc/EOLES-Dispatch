...
def build_model(run_dir):
    ...
    eff50 = pd.read_csv(input_dir / "eff50.csv", header=None, names=["thr", "eff50"]).set_index("thr")
    ...
    su_fixedCost = pd.read_csv(input_dir / "su_fixedCost.csv", header=None, names=["thr", "su_fixedCost"]).set_index("thr")
    su_fuelCons = pd.read_csv(input_dir / "su_fuelCons.csv", header=None, names=["thr", "su_fuelCons"]).set_index("thr")
    ramp_fuelCons = pd.read_csv(input_dir / "ramp_fuelCons.csv", header=None, names=["thr", "ramp_fuelCons"]).set_index("thr")
    ...
    minSG = pd.read_csv(input_dir / "minSG.csv", header=None, names=["thr", "minSG"]).set_index("thr").squeeze(axis=1)
    minTimeOFF = pd.read_csv(input_dir / "minTimeOFF.csv", header=None, names=["thr", "minTimeOFF"]).set_index("thr").squeeze(axis=1)
    minTimeON = pd.read_csv(input_dir / "minTimeON.csv", header=None, names=["thr", "minTimeON"]).set_index("thr").squeeze(axis=1)
    ...
    model.gene_on_hmax_constraint = pyo.Constraint(model.a, model.thr, model.h, rule=gene_on_hmax_rule)

    def gene_on_hmin_rule(model, a, thr, h):
        return model.on[a, thr, h] * minSG[thr] <= model.gene[a, thr, h]
    model.gene_on_hmin_constraint = pyo.Constraint(model.a, model.thr, model.h, rule=gene_on_hmin_rule)
    ...
    def cons_startup_rule(model, a, thr, h):
        if h - minTimeOFF[thr] >= model.h.first():
            ...
            recently_off = itertools.chain(
                ...
                range(model.h.last() - minTimeOFF[thr] + (h - model.h.first()) + 1, model.h.last() + 1),
            )
    ...
    model.cons_startup_constraint = pyo.Constraint(model.a, model.thr, model.h, rule=cons_startup_rule)
    ...
    def cons_turnoff_rule(model, a, thr, h):
        if h - minTimeON[thr] >= model.h.first():
            ...
            recently_on = itertools.chain(
                range(model.h.first(), h),
                range(model.h.last() - minTimeON[thr] + (h - model.h.first()) + 1, model.h.last() + 1),
            )
        return model.turnoff[a, thr, h] <= model.on[a, thr, h] - sum(model.startup[a, thr, h_bis] for h_bis in recently_on)
    model.cons_turnoff_constraint = pyo.Constraint(model.a, model.thr, model.h, rule=cons_turnoff_rule)
