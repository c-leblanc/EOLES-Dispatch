...
def main():
    ...
    create_parser = subparsers.add_parser("create",
                                           help="Create a new run (fetch data, format inputs)")
    ...
    create_parser.add_argument("--no-download", action="store_true",
    ...
    solve_parser.add_argument("--model-version", default="standard",
                               help="Model version (default: standard)")
    solve_parser.add_argument("--reports", nargs="+", default=["prices", "production"],
    ...
    collect_parser = subparsers.add_parser("collect",
                                           help="Download data from ENTSO-E and/or Renewables.ninja")
    collect_parser.add_argument("--start", type=int, required=True,
                                help="First year to download (e.g. 2020)")
    collect_parser.add_argument("--end", type=int, required=True,
                                help="Last year (exclusive, e.g. 2025 for 2020-2024)")
    collect_parser.add_argument("--source", default="all", choices=["all", "entsoe", "ninja"],
    ...
    viz_parser = subparsers.add_parser("viz",
                                        help="Generate an interactive HTML report for a run")
    ...
    conv_parser = subparsers.add_parser("convert-scenario",
                                        help="Convert an Excel scenario to a CSV directory")
    conv_parser.add_argument("xlsx_path", type=Path, help="Path to the .xlsx scenario file")
    conv_parser.add_argument("--output-dir", type=Path, default=None,
    ...
    if args.command == "create":
        ...
        if not runs:
            ...
            print("-" * 85)
            for r in runs:
                print(f"{r.get('name','?'):<30} {r.get('scenario','?'):<15} "
