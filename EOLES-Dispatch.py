...
genCarb = efficiency.join(co2_factor,how='left')
genCarb['genCarb'] = (1/genCarb.efficiency)*GJ_MWh*genCarb.co2_factor
...
def storing_constraint_rule(model, a, sto, h):
    ...
    h_next = h+1 if h<model.h.last() else model.h.first()
    if sto=='lake_phs':
        return model.stored[a,sto,h_next] == model.stored[a,sto,h] + model.storage[a,sto,h]*eta_in[sto]*eta_out[sto] - model.gene[a,sto,h] + lake_inflows[a,hours_months[h]]*1000/len(months_hours[hours_months[h]])
    else :
        return model.stored[a,sto,h_next] == model.stored[a,sto,h] + model.storage[a,sto,h]*eta_in[sto]*eta_out[sto] - model.gene[a,sto,h]
...
def hcost_definition(model, a, h):
    ...
    return model.hcost[a,h] == \
        ...
        sum((model.exo_im[a,exo_a,h]-model.exo_ex[a,exo_a,h])*exoPrices[exo_a,h] for exo_a in model.exo_a) + \
        model.hll[a,h]*voll
...
### OUTPUTS EXPORTS
# Exports production per technology, wholesale prices and trade between France and other countries
# NB: All outputs are not exported - Additional exports can be parameterized here

# Store hourly production per technology category in each country in the <production> data frame
...
demand.index.names = ['area','hour']
production = production.join(demand)

# Store hourly prices in each country in the <prices> data frame
...
# Store net imports to France from each neighboring country in the <FRtrade> data frame
...
# Save outputs as .csv files in the <./outputs> directory
